...
#base_model = VGG16()
base_model = ResNet50()
# imports the base Resnet50 model and stores it in base_model along with the preloaded
ImageNet weights
base_model.summary()
...
#add a fully connected layer with 1000 neurons to the fl1 layer and apply relu activation on itdrop1=Dropout(0.3)(fc1)
#add a dropout of 0.3 i.e kill 30% of the neurons at random to introduce generecity and avoid
overfitting
fc2=Dense(500,activation='relu')(drop1)
drop2=Dropout(0.3)(fc2)
# add the fully connected layer with 500 neurons and apply a dropout of 0.3 same as the
previous layer
...
# since resnet is already trained on the imagenet dataset it has already learned the basic
features
# such as lines , curves etc and thus we freeze the resnet model to avoid computation and save
time
...
def get_input():
X=[]
label=[]path1="cat-and-dog/cats"
path2="cat-and-dog/dogs"
#path3="aug_high_myopia"
...
# split the data as 20% testing and 80% training with a random state of 2. Random state means
# everytime the data is randomly shuffled and then split but using the same random state means
...
#checkpoint=ModelCheckpoint("Model.h5",monitor='val_acc',mode='max',save_best_only=True,
...
# mode defines when we have to save the weights when the thing being monitored is maximum
or minimum
# save_best_only set to True means that only the best model is saved and save_weights_only
means that
# only the model weights are stored since we have already stored the json file# **** it is always recommended to save the json and weights independently instead of the
whole model at once *****
...
# X_test and y_test are the validation data on which the trained model will be tested after every
epoch
